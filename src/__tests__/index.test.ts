...
describe("semantic layer", async () => {
  ...
  describe("models from tables", async () => {
    ...
    it("will remove non projected members from order clause", async () => {
      ...
      // Sort the rows on the client, because they will be sorted by customer id which is same for all rows
      assert.deepEqual(
        result.rows.sort(),
        [
          {
  ...
  describe("full repository", () => {
    ...
    it("should validate filters so or/and connectives include filters with members of the same type", () => {
      ...
      assert.deepEqual(result1.error.issues, [
        {
          code: "custom",
          message:
            "All and/or connectives must include filters with members of the same type (dimension or metric)",
          path: ["filters", 0, "filters"],
        },
      ]);

      const result2 = queryBuilder.querySchema.safeParse({
        members: ["invoice_lines.unit_price"],
        filters: [
          {
            operator: "or",
      ...
      assert.deepEqual(result2.error.issues, [
        {
          code: "custom",
          message:
            "All and/or connectives must include filters with members of the same type (dimension or metric)",
      ...
      const result3 = queryBuilder.querySchema.safeParse({
        members: ["invoice_lines.unit_price"],
        filters: [
          {
            operator: "or",
      ...
      // In this case all parent connective filters are automatically invalid because a child connective filter is invalid
      assert.deepEqual(result3.error.issues, [
        {
          code: "custom",
          message:
            "All and/or connectives must include filters with members of the same type (dimension or metric)",
          path: ["filters", 0, "filters", 0, "filters", 0, "filters"],
        },
        {
          code: "custom",
          message:
            "All and/or connectives must include filters with members of the same type (dimension or metric)",
          path: ["filters", 0, "filters", 0, "filters"],
        },
        {
          code: "custom",
          message:
