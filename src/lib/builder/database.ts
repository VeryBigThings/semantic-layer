...
type TableTN<T> = T extends Table<infer TN, any, any> ? TN : never;
// biome-ignore lint/suspicious/noExplicitAny: <explanation>
type TableDN<T> = T extends Table<infer TN, infer DN, any>
  ? `${TN}.${DN}`
  : never;
// biome-ignore lint/suspicious/noExplicitAny: <explanation>
type TableMN<T> = T extends Table<infer TN, any, infer MN>
  ? `${TN}.${MN}`
  : never;

class DimensionRef<TN extends string, DN extends string> {
  constructor(
  ...
  render(database: Database, dialect: BaseDialect) {
    ...
    for (let i = 0; i < this.strings.length; i++) {
      ...
      if (this.values[i]) {
        ...
        if (value instanceof DimensionRef) {
...
type TableDimensionsWithoutTablePrefix<
  TN extends string,
  DN extends string,
...
> = {
  [TK in TN1]: {
    [DK in TableDimensionsWithoutTablePrefix<TN1, DN>]: DimensionRef<TK, DK>;
  };
} & {
  [TK in TN2]: {
    [DK in TableDimensionsWithoutTablePrefix<TN2, DN>]: DimensionRef<TK, DK>;
  };
};
  ...
  join<TN1 extends string, TN2 extends string>(
    ...
    const dimensions = {
      [table1.name]: Object.keys(table1.dimensions).reduce<
        Record<string, DimensionRef<string, string>>
      >((acc, dimension) => {
        acc[dimension] = new DimensionRef(tableName1, dimension);
        return acc;
      }, {}),
      [table2.name]: Object.keys(table2.dimensions).reduce<
        Record<string, DimensionRef<string, string>>
      >((acc, dimension) => {
        acc[dimension] = new DimensionRef(table2.name, dimension);
        return acc;
      }, {}),
